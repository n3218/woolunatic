...
export const listProductsInCategory = (category = "") => async dispatch => {
  try {
    dispatch({ type: PRODUCT_LISTINCATEGORY_REQUEST })
    const { data } = await axios.get(`/api/products?category=${category}`)
    dispatch({ type: PRODUCT_LIST_SUCCESS, payload: data })
    dispatch({ type: PRODUCT_LISTINCATEGORY_SUCCESS, payload: data })
  } catch (error) {
    dispatch({ type: PRODUCT_LISTINCATEGORY_FAIL, payload: error.response && error.response.data.message ? error.response.data.message : error.message })
  }
}
...
export const productTopAction = () => async dispatch => {
  try {
    ...
    console.log("productTopAction: data: ", data)
    dispatch({
