...
export const sendMail = async orderData => {
  ...
  const row = item => {
    ...
    <td style="text-align: center;">
      <a target="_blank" rel="noreferrer" href="https://woolunatic.herokuapp.com/products/${item.product}" style="text-decoration:none; color:#417d97">${item.color.replace(/_+/g, " ")}</a>
    </td>
  ...
  const mailOptions = {
    ...
    <div style="color: #373a3c; font-family: 'Source Sans Pro',Roboto,'Helvetica Neue',Arial,sans-serifs; font-weight: 300; background-color: #f7f7f7; padding: 20px;">
      <div style="max-width: 700px; margin: 0px auto; background-color: white; padding: 10px;">
        ...
        <div style="font-size: 20px; margin-bottom: 20px;"><a href="https://woolunatic.herokuapp.com/orders/${order._id}" style="text-decoration:none; color:#417d97" target="_blank" rel="noreferrer">#${order._id}</a> Order confirmation</div>
            ...
            <table>
              <tbody>
                ...
                <tr><td style="text-align: right; font-size: 12px; font-weight: 300;">payment status: </td><td style="font-size: 12px; font-weight: 300;"> 
                  <div style="background-color:${badges[order.paymentResult.status]} text-transform:uppercase; display:inline-block">${order.paymentResult.status}</div>
                </td></tr>
              </tbody>
            </table>
          </div>
        ...
        <div>
          <div style="font-size: 18px; font-weight: 300;" align="left">ORDER DETAILS (total ${order.orderItems.length} items):</div>
        ...
        <div align="right" style="padding-top: 10px; background-color: #f7f7f7;">
          <table cellspacing="0">
            <tbody>
